...
export default function Home() {
  ...
  return (
      <div className="min-h-screen bg-radial-gradient text-white w-full">
        ...
        {/* Platforms Section */}
        <section className="py-16 px-0 sm:px-6 md:px-12 lg:px-24">
          <div className="max-w-6xl mx-auto flex flex-col items-center justify-center">
            <h2 className="text-3xl font-bold text-center mb-4 px-8">
              다양한 플랫폼의 트렌드를 한곳에서
            </h2>
            <p className="text-center text-gray-300 mb-12 max-w-2xl mx-auto px-8">
              글로벌/국내 주요 플랫폼의 데이터를 수집하여 가장 정확하고 신선한 트렌드 정보를 제공합니다.
            </p>

            {/* 슬라이드 컨테이너 */}
            <div className={`relative overflow-hidden sm:bg-black/30 backdrop-blur-sm sm:border sm:border-gray-700/50 hover:border-gray-600/50 rounded-2xl
              ${isCollapsed ? 'w-[calc(100lvw)] sm:w-[calc(100lvw-18rem)] md:w-[calc(100lvw-18rem)] lg:w-[calc(100lvw-18rem)]' : 'w-[calc(100lvw)] sm:w-[calc(100lvw-28rem)] md:w-[calc(100lvw-28rem)] lg:w-[calc(100lvw-28rem)]'}
                `}>
              {/* 그라데이션 마스크 - 좌우 페이드 효과 */}
              <div className="absolute left-0 top-0 w-24 h-full bg-gradient-to-r sm:from-black/60 to-transparent z-10 pointer-events-none"></div>
              <div className="absolute right-0 top-0 w-24 h-full bg-gradient-to-l sm:from-black/60 to-transparent z-10 pointer-events-none"></div>
                ...
                <div className="flex gap-6 px-3">
                  {['YouTube', 'Reddit', 'Twitter', 'Google', 'Naver', 'Instagram'].map((platform) => (
                      <div key={`${platform}-1`} className="flex-shrink-0">
                        <div className="px-2 sm:px-8 pt-7 pb-5 transition-all">
        ...
        <section className="py-16 px-6 md:px-12 lg:px-24 bg-gradient-to-r from-primary to-secondary text-white">
          <div className="max-w-4xl mx-auto text-center">
            ...
            <div className="flex flex-wrap gap-4 justify-center">
              <Link
                  href="/dashboard"
                  className="px-8 py-4 bg-white text-primary rounded-lg font-bold transition-colors shadow-lg hover:shadow-xl"
              >
                무료로 시작하기
              </Link>
              <Link
                  href="/developers"
                  className="px-8 py-4 bg-transparent border border-white hover:bg-white/10 text-white rounded-lg font-bold transition-colors"
              >
                개발자 API 살펴보기
              </Link>
            </div>
